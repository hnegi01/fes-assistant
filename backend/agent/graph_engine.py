"""
LangGraph engine — the DEFAULT harness for the agent turn.

Selected by ``FES_AGENT_ENGINE`` (default ``langgraph``; ``custom`` is the
dependency-free kill switch that keeps the hand-rolled ``_reactive_loop``).
Both engines are thin control flow over the SAME organs in ``llm_agent`` —
planner, router, tool selection, validation, mutation gate, execution,
decide/replan, critic, finalize — accessed here via module attributes
(``A.<helper>``) so unit-test monkeypatches on ``llm_agent`` apply identically
...
def _done_reply(s: GraphState, answer: str) -> str:
    ...
    return answer + hint + tail_note + A._followup_tail()
...
def _record_execution(
    ...
    A._record_step(steps_executed + 1, tool_id, result)
...
async def node_first_select(s: GraphState) -> Dict[str, Any]:
    ...
    if s["mode"] == "migration":
        ...
        if nav_pkg == "__unclear__":
            if s.get("resume_clarification"):
                ...
                return {"reply": reply}
            if A._planner_text_worth_surfacing(first_op, s["user_text"]):
                # The planner wrote a message to the user instead of a plan —
                # its text IS the reply; the canned line would discard it
                # (live 2026-08-20).
                _write_trace(s, "planner_text_reply", with_steps=False)
                return {"reply": first_op.strip()}
    ...
    try:
        ...
        A._record_tool_result(result)
        A._record_step(1, result.get("tool_id", "fallback"), result)
        A._write_llm_trace(s["trace"])
