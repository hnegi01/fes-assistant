...
MIGRATION_PLAN_SYSTEM_PROMPT, not in a rank table here. A table keyed on
    ...
    *,
    ...
    if calls is None:
        if seed_call is not None:
            ...
            try:
                # tool_choice="auto", NOT the tools-imply-"required" default: the
                # prompt tells the planner to answer in prose (and plan nothing)
                # when the request is not a move-existing-assets migration —
                # "required" forbids exactly that, forcing the least-wrong tool
                # call instead ("create datamodel X" gated migrate_datamodels,
                # live 2026-08-21). The no-calls branch below is that path.
                data = await A.call_llm_raw(
                    messages, tools=passed_tools, trace_id=turn_trace_id, tool_choice="auto", label="migration_plan"
                )
            ...
            if resume_clarification and all(
                ...
                # something that planned into DIFFERENT operations: a topic change
                # ("migrate all datamodels" while dashboard-shares clarification was
                # pending, live 2026-08-17). Drop the stale clarification and let the
                # NEW plan gate normally — the approval dialog is the safety net.
                # History: this used to RE-ASK (2026-08-10: a question about the
                # clarification became a divergent plan), but that harm is now
                # absorbed twice over — questions that plan nothing take the
                # answer-then-re-ask branch above, and a divergent plan only ever
                # reaches the DIALOG, never execution. Punishing every genuine topic
                # change with extra turns protected against nothing the gate doesn't.
                logger.info(
                    "Clarification resume planned %s instead of the pinned %s — topic change; "
                    "dropping the stale clarification and gating the new plan.",
                    [_tool_id_of(c) for c in calls],
                    resume_clarification.get("tool_id"),
                )
                resume_clarification = None
        ...
        for call in calls:
            ...
            try:
                ...
                if missing:
                    ...
                    # placeholder must not be echoed back as "you gave me this".
                    filled = {k: v for k, v in args.items() if A._is_filled(v) and k not in missing}
    ...
    for idx, call in enumerate(calls):
        ...
        A._record_tool_result(result)
        A._record_step(step_number, tool_id, result)
