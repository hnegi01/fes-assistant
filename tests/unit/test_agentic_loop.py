...
# ---------------------------------------------------------------------------
# Routing-unclear on planner-authored text: surface the planner's message
# (live 2026-08-20: a stale clarify exchange made the planner role-play the
# assistant; its question became the step text, routing said unclear, and the
# canned fallback discarded a useful reply)
# ---------------------------------------------------------------------------

PLANNER_QUESTION = (
    'I need more details to proceed with creating the data model "abc." '
    "Specifically, I need the connection to use. Could you provide these details?"
)


class TestPlannerTextSurfacing:
    def test_helper_accepts_question_shaped_planner_text(self):
        assert m._planner_text_worth_surfacing(PLANNER_QUESTION, "create datamodel abc")

    def test_helper_rejects_users_own_words(self):
        # Unclear routing on the user's own message is genuine gibberish —
        # the canned fallback is the right answer there.
        assert not m._planner_text_worth_surfacing("asdf qwerty?", "asdf qwerty?")

    def test_helper_rejects_mangled_imperatives(self):
        # Planner-authored but not a message to the user: echoing it back
        # would read as nonsense.
        assert not m._planner_text_worth_surfacing("Provision the abc schema entity", "create datamodel abc")

    def test_helper_rejects_empty(self):
        assert not m._planner_text_worth_surfacing("", "create datamodel abc")

    def test_unclear_routing_surfaces_planner_question(self):
        # History is load-bearing: with none, the single-step faithfulness
        # guard replaces the plan with the user's words and the canned fallback
        # is correct. The live incident had a stale clarify exchange in history,
        # which is exactly when the planner drifts into writing a question.
        client = _fake_client()
        messages = [
            {"role": "user", "content": "set up a new extract datamodel called UI_TEST_MODEL"},
            {"role": "assistant", "content": "I need a bit more information to run this."},
            {"role": "user", "content": "create datamodel abc"},
        ]
        nav = AsyncMock(side_effect=[([], "__unclear__", None, 0)])
        raw = AsyncMock(side_effect=[])  # neither planner (patched) nor selection (unclear) reach the LLM

        async def _question_plan(user_text, mode, history, trace_id):
            return [PLANNER_QUESTION]  # the drifted planner: a message to the user as the "plan"

        with (
            patch.object(m, "_make_plan", new=_question_plan),
            patch.object(m, "_navigate_to_tools", new=nav),
            patch.object(m, "call_llm_raw", new=raw),
        ):
            reply = run(m.call_llm_with_tools(messages, GET_USER_TOOLS, client, allow_summarization=True))
        assert reply.strip() == PLANNER_QUESTION
        client.invoke_tool.assert_not_awaited()

    def test_unclear_routing_on_user_gibberish_keeps_canned_fallback(self):
        client = _fake_client()
        reply, _nav, _raw = _run_turn(
            llm_responses=[_text_resp("asdf qwerty")],
            nav_side_effect=[([], "__unclear__", None, 0)],
            client=client,
            user_msg="asdf qwerty",
        )
