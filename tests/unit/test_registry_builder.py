...
class TestSummaryOverrides:
    ...
    def test_override_is_deleted_once_the_sdk_says_it_itself(self):
        import inspect

        from scripts.registry_core import MODULES

        for tool_id, override in _builder()._SUMMARY_OVERRIDES.items():
            module_name, method_name = tool_id.split(".", 1)
            doc = (inspect.getdoc(getattr(MODULES[module_name], method_name)) or "").strip()
            first_line = (doc.splitlines()[0] if doc else "").strip()
            assert first_line != override, (
                f"{tool_id}: the installed SDK's own first line now matches the override — "
                "delete the entry from _SUMMARY_OVERRIDES and regenerate the registry (scripts/01)"
            )
    ...
    def test_shipped_registry_carries_the_overrides(self):
        ...
        for tool_id, override in _builder()._SUMMARY_OVERRIDES.items():
            ...
            assert by_id[tool_id]["description"] == override, (
