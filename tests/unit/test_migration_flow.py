...
def test_an_explicit_empty_list_for_a_required_argument_also_asks():
    """The model says "the user named nothing" two ways: omitting the key, or
    emitting an explicit []. The empty list is schema-valid (`required` only
    demands the key EXISTS), so it used to sail past validation into the
    approval dialog — seen live 2026-08-18: migrate_dashboard_shares gated
    with blank source/target id lists. Both spellings must clarify."""
    client = _client()
    with patch.object(
        m, "_generate_clarification_question", new=AsyncMock(return_value="Which groups should I migrate?")
    ):
        reply, _ = _turn(
            [_multi_resp(_call("migration.migrate_groups", {"group_name_list": []}, "c1"))],
            client,
        )
    client.invoke_tool.assert_not_awaited()
    assert reply == "Which groups should I migrate?"
    assert m.LAST_PENDING_CLARIFICATION["tool_id"] == "migration.migrate_groups"
    assert not (m.LAST_TOOL_RESULT or {}).get("pending_confirmation")
...
class TestDialogReadability:
    ...
    def test_list_suffix_dropped_and_singular_for_one_value(self):
        ...
        2026-08-14: 'group name list: sales_test_2' reads weird)."""
        assert mf._humanise_args({"group_name_list": ["sales_test_2"]}) == "group name: sales_test_2"
...
def test_a_topic_change_during_clarification_gates_the_new_plan():
    """A NEW request while a clarification is pending drops the stale
    clarification and gates the new plan in ONE turn (live 2026-08-17:
    'migrate all datamodels' while the dashboard-shares clarification was
    pending got re-asked about dashboards — the old guard punished every
    topic change). The 2026-08-10 harm the old guard existed for (a QUESTION
    hallucinating a divergent plan) is absorbed by the gate itself: the
    divergent plan only ever reaches the approval dialog, never execution —
    and questions that plan nothing still take the answer-then-re-ask branch
    (tests below)."""
    import backend.agent.migration_flow as mf2
    ...
    with patch.object(m, "call_llm_raw", new=raw):
        run(
            mf2.run(
                resume_clarification=resume,
                **_flow_kwargs(client, user_text_msg="migrate all datamodels to the target environment"),
            )
        )

    client.invoke_tool.assert_not_awaited(), "gated, not executed — the dialog is the safety net"
    pending = (m.LAST_TOOL_RESULT or {}).get("pending_confirmation") or {}
    steps = (pending.get("arguments") or {}).get("steps") or []
    assert [s.get("tool") for s in steps] == ["migration.migrate_all_datamodels"], "the NEW plan gates"
    assert m.LAST_PENDING_CLARIFICATION is None, "the stale clarification is dropped, not re-asked"
...
def test_a_question_about_the_clarification_gets_an_answer_before_the_reask():
    ...
    raw = AsyncMock(
        side_effect=[
            # The planner declines to plan for a question (the prompt tells it
            # to answer in prose when no asset is named) — the NO-CALLS path.
            # A planner that instead hallucinates a divergent plan now gates it
            # (see test_a_topic_change_during_clarification_gates_the_new_plan).
            _text_resp("That's a question about a setting, not a migration request."),
...
def test_the_answer_call_failing_still_reasks():
    ...
    raw = AsyncMock(
        side_effect=[
            _text_resp("That's a question, not a migration request."),  # no-calls path
