...
def test_topic_change_on_resume_routes_fresh(backend_url, tenant_config):
    ...
    # A read-only ask that clarifies deterministically (see test_clarification_flow
    # for why the "by their email address" phrasing is unsuitable here).
    first = _turn(backend_url, tenant_config, session_id, "show me the columns of a datamodel", history)
