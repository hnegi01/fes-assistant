...
EVAL_CASES = [
    ...
    {
        "id": "create-request-is-not-coerced-into-a-migration",
        "prompt": "create datamodel fes_assistant use connection pysense_databricks "
        "use database samples and schema as nyctaxi and add table trips",
        # A CREATE request typed in migration mode (user picked the wrong tab).
        # Migration mode has no create tools; the answer is never the nearest
        # migrate call — approving it would MIGRATE the same-named existing
        # asset, not create anything. Correct: no plan, decline in prose,
        # point to Chat mode. Two guards behind this: the prompt invariant
        # (plan only move-existing-assets requests) and tool_choice="auto" on
        # the plan call ("required" forced a least-wrong tool call, making the
        # prose-decline branch unreachable).
        "expect_steps_any": [],
        "forbid_steps": [
            "migration.migrate_datamodels",
            "migration.migrate_all_datamodels",
        ],
        "expect_reply_any": ["chat mode"],
        "forbid_reply": [],
        "origin": "2026-08-21: live V2 pass — create request in migration mode gated "
        "migrate_datamodels(['fes_assistant']) with an invented provider_connection_map.",
    },
    {
        "id": "off-topic-request-is-denied-not-planned",
        "prompt": "what is the temperature today in NJ",
        # Entirely non-Sisense request: deny it — never a plan. Under the old
        # forced-tool-call contract this gated "migrate all eligible users".
        "expect_steps_any": [],
        "forbid_steps": [
            "migration.migrate_all_users",
            "migration.migrate_users",
        ...
        "expect_reply_any": ["chat mode", "migrat"],
        "forbid_reply": ["approve"],
        "origin": "2026-08-21: forced tool_choice turned a weather question into a gated migrate-all-users plan.",
    },
]
